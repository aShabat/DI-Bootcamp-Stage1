...
def unzip_with_7z(zip_file_path, dest_path, password):
    ...
    if os_type == "Darwin":  # macOS
        result = subprocess.run(
            ["7za", "x", "-p" + password, "-o" + dest_path, zip_file_path],
        ...
        result = subprocess.run(
            ["7z", "x", "-p" + password, "-o" + dest_path, zip_file_path],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
        )
    ...
    if "Everything is Ok" in cmd_output:
        print(
            f"Successfully extracted all files to {dest_path} with password {password}"
        )
        return True
    else:
        return False
